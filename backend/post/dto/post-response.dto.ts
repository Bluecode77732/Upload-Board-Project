...
export class PostResponseDto {
  @ApiProperty({ description: '게시글 id. (Post id.)' })
  id!: number;

  @ApiProperty({ description: '게시글 제목. (Post title.)' })
  title!: string;

  @ApiProperty({ description: '게시글 본문. (Post body.)' })
  body!: string;
  ...
  @ApiPropertyOptional({
    ...
    description:
      '텍스트 전용 게시글에는 없다. URL 조립이 한 곳에서만 이뤄지도록 FileService가 ' +
      '만들어 넣는다(ADR 0023). (Absent on a text-only post. Built by FileService so ' +
      'URL assembly stays in one place (ADR 0023).)',
  })
