...
export class AuthController {
  ...
  @ApiOperation({ description: "Register with Basic Token (base64 email:password)" })
  ...
  @ApiResponse({ status: 401, description: "Unauthorized. Provide the refresh token from /auth/signin." })
  async refreshAccessToken(@Headers("authorization") rawToken: string) {
    ...
    return {
      accessToken: await this.authService.issueToken({ id: payload.sub }, false),
    };
  };
  ...
  @ApiResponse({ status: 201, description: "Issues refresh and access tokens.", type: tokenType })
  @ApiResponse({ status: 401, description: "Invalid credentials." })
