...
| **S3** | 오브젝트 스토리지 | 🔶 어댑터 ✅ / 리다이렉트 설계 📝 / 컷오버 🆕 | `FileStorage` 포트 + `S3Storage` 구현 랜딩(단위테스트만, 실버킷 미실행). 프록시 스트리밍 경로가 앱 계층에 대역폭 부담을 지우고 있어, `GET /file/:id/content`에 대한 presigned URL 리다이렉트(세 가시성 등급 전부, 기존 `resolveContentAccess` 검사로 게이트)를 설계했다 — 아직 코드는 없음. 잔여: 리다이렉트 구현 후 실버킷 **`STORAGE_DRIVER=s3` 전환**. `local`이 운영 기본값 유지. | [0029](ADR/0029-storage-port-adapter.ko.md), [0036](ADR/0036-s3-presigned-content-redirect.ko.md) |
