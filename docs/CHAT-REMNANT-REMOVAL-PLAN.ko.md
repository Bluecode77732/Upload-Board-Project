...
**상태: 대부분 종결** — 2026-07-22 검토 실행 완료; git 히스토리 결정(계획서 1번
항목)은 2026-09-07에 내려짐. 남은 건 2번 항목인 재검증 트리거뿐 — 지금까지 네 번
수동으로 발동했지만 아직 자동화되지 않아 계획서 자체의 완료 기준상 "종결"은 아님.
...
## 재검증 — 개발자 요청에 의한 전체 범위 (2026-09-09)

개발자가 직접 재확인을 요청한 것이 발단입니다("현 앱 트리 구성 요소 중 잔재 요소 조사
후 보고, 재진행") — 잔여 작업 2번 항목이 나열한 세 조건(새 문서 파일 추가 / 다른
프로젝트나 과거 브랜치에서 붙여넣기 / 공개 직전) 중 어느 것에도 해당하지 않습니다.
2번 항목 목록에 조용히 끼워 넣지 않고 네 번째 별도 발동으로 기록합니다 — "개발자가
직접 요청"도 이 계획서가 아직 이름 붙이지 않았던 실제 트리거였기 때문입니다.

방법: 세트 A/B를 처음부터 다시 훑는 대신, 마지막 전체 범위 검토(`63ce1a9`,
2026-09-07) 이후 변경된 파일만 diff로 뽑아 두 세트를 그 위에 실행했습니다 — 처음부터
다시 훑으면 `CLAUDE.md`의 Concern-to-entrypoint map 자체가 거의 모든 줄에 `backend/`·
`admin/`·`frontend/` 같은 경로 용어(버킷 3, 고유 기능)를 정당하게 담고 있어 잡음이
결과를 뒤덮기 때문입니다. 이번 재검증에서는 저장소 외부 메모리 파일(3번 항목, 마지막
확인 2026-07-22)도 함께 훑었습니다.

`63ce1a9` 이후 변경된 파일은 62개(`git diff --name-only`), 그중 신규 추가는 5개:
`admin/src/components/theme-toggle.tsx`, `admin/src/store/theme.store.ts`(+`.spec.ts`),
`frontend/src/features/account/SettingsPage.tsx`(+`.module.css`). 두 세트 모두 변경된
파일 전체를 대상으로 실행했습니다.

**발견된 잔재: 0건.** 모든 검색 결과가 위 2026-08-13/2026-09-07 표에서 이미 확립된
분류에 그대로 들어맞았습니다 — 새로운 패턴은 없습니다:

| 검색 결과 | 위치 | 분류 | 조치 |
|---|---|---|---|
| `superadmin`, `AuditLog`, `zustand`(코드) | `CLAUDE.md`(.ko)의 entrypoint map·RBAC 절, `admin/src/store/theme.store.ts`의 import 경로뿐 | 고유 기능 | 유지 |
| 2026-07-30 이식과 2026-08-06 적응 작업을 서술하는 "Chat Project" 산문(`rooms-page`, Apollo/`/graphql`, ban/force-logout을 *삭제된* 항목으로 언급) | `admin/README.md`(.ko), `docs/CHANGELOG.md`(.ko), `docs/ROADMAP.md`(.ko) | 설계 참조 | 유지 — 전부 과거 시제로 무엇이 삭제됐는지 서술할 뿐, 이 저장소의 현재 스택을 서술하지 않음 |
| `winston`/`Sentry` | `CLAUDE.md`(.ko) | 부정문 | 유지 |
| "chat-project git-history remnant handling", "chat response" | 메모리(`session-prompts.md`, `ask-with-comparison-tables.md`) | 오탐 / 자기 참조 | 유지 — 앞의 것은 이 계획서 1번 항목 그 자체(이미 해결됨)를 가리키고, 뒤의 것은 무관한 산문 |

신규 추가된 5개 파일(테마 토글, 테마 스토어, Settings 페이지)은 두 세트 모두에서
결과가 0건이었습니다 — 함께 들어온 잔재 용어는 없습니다. `.env.example`은 변경된
파일 목록에 없어 2026-09-07 결과(두 세트 모두 0건)를 재검증 없이 그대로 이어받습니다.
메모리 파일: 2026-07-22와 마찬가지로 클린.

버킷 1 결과는 없습니다. 이 저장소는 이번 패스 기준으로 HEAD에서 여전히 깨끗합니다 —
네 번째 연속 클린 전체 범위 재검증입니다.

## 잔여 작업 (처리 예정)

1. ~~**Git 히스토리 결정**~~ — **2026-09-07 결정: 현상 유지.** `4d00bc2` 이전
   커밋들에는 채팅 앱 `CLAUDE.md`가 여전히 남아 있습니다(`git show c8eb19f:CLAUDE.md`로
   열람 가능). 계획서 자체가 권장하던 옵션 그대로 선택됨: 정직한 이력 기록이며,
   히스토리 재작성(`filter-repo`)은 파괴적이고 `CHANGELOG.md`/`ROADMAP.md`에 인용된
   모든 커밋 해시를 깨뜨립니다(재작성이 정당화되는 유일한 경우 — 과거 내용이
   공개되어서는 안 되는 경우 — 도 여기엔 해당 안 됨, 설계 문서·코드일 뿐 비밀·개인정보가
   아닙니다). 2026-09-07 전체 범위 재검증에서 HEAD가 어차피 클린함을 재확인했습니다.
   필요할 때만 구현하는 방식으로 정리: 지금은 예상 못 한 새로운 이유가 실제로
   생기지 않는 한 재검토하지 않습니다 — 선제적으로는 안 함.
2. **재검증 트리거** — 다음 시점마다 위 방법의 grep 세트를 재실행:
   - 새 문서 파일이 추가될 때
   - 다른 프로젝트나 과거 브랜치에서 내용을 붙여넣을 때
   - 저장소를 공개/태깅하기 직전
   - 개발자가 직접 재확인을 요청할 때(2026-09-09 추가 — 네 번째 발동이 앞의 세
     조건 어디에도 맞지 않았습니다)

   지금까지 4회 발동 — `admin/` 이식(2026-07-30, 문서 범위), 코드 범위 재검증
   (2026-08-13, 첫 검토가 남긴 문서 전용 공백을 닫음), "공개 직전" 조건
   (2026-09-07, 예상해서 대비한 게 아니라 이미 발동한 걸 뒤늦게 발견함 — 아무도 이
   방법을 다시 실행해보기 전까지 저장소는 이미 한동안 공개 상태였습니다), 그리고
   개발자의 직접 요청(2026-09-09, 두 세트를 처음부터 재실행하는 대신 직전 패스 대비
   diff로 대체 — 이유는 위 해당 절 참고). 앞으로의 모든 검토는 범위를 문서 전용이나
   특정 하위폴더로 되돌리지 말고 계속 넓혀야 합니다.
