...
- **`ROADMAP.md`(+ko): §7의 낡은 "미착수" 항목 2건 추가 정정 (2026-09-08)** — 앞서
  ...
  인프라/미확정 사항 때문에 막혀 있다"고 적혀 있었는데, §6의 Stage 4 상태표는
  이미 둘 다 반영되고 라이브 검증까지 됐다고 기록하고 있었다(ADR 0043
  D4/D5/D7, 2026-08-18) — Ingress 쪽은 특히 빠진 의존성이 아니라 개발자의
  **의도적 선택**(2026-08-27 확정)으로 `enabled: false`인 것이었다. 앞서 논의한
  "인프라 조건부 보류" 4항목 중 나머지 둘(Testcontainers 등)처럼 "필요 시 구현"
  확정 문구가 필요한지 재확인하다가 발견함 — 넷 중 둘은 애초에 그런 프레이밍
  자체가 불필요했다, 원래 막혀 있던 적이 없었으니까. §7이 §6을 못 따라간 것뿐.
  코드 변경 없음 — 순수 문서.
...
- **오래 대기 중이던 백로그 3건을 "필요할 때만 구현"으로 정리 (2026-09-07)** —
  ...
  - **Chat 프로젝트 잔재 git 히스토리**: 계획서(`docs/CHAT-REMNANT-REMOVAL-PLAN.md`
    1번 항목)가 권장하던 그대로 **현상 유지** 결정 — 재작성은 파괴적이고
    `CHANGELOG.md`/`ROADMAP.md`가 이미 인용 중인 커밋 해시를 전부 깨뜨리며, 재작성이
    정당화되는 유일한 경우(공개 불가 내용)도 여기엔 해당 안 됨. 진짜 새로운 이유가
    생길 때만 재검토.
  - **e2e용 Testcontainers**: 계속 유예하되 근거를 갱신 — 원래 트리거("Stage 4 배포
    환경 확정")는 이미 지나갔는데 그동안 지금의 수동 Postgres 방식이 아무 문제도
    일으키지 않았다. 실제로 마찰(두 번째 개발자, CI 불안정)이 생길 때만 도입하는
    쪽으로 재정리.
  - **`pnpm audit` dev-transitive 지적**: 이 코드베이스에서 할 수 있는 조치가 없음을
    재확인 — 적용할 수정 자체가 없고 업스트림 릴리스를 기다리는 것뿐.
  코드 변경 없음 — `docs/ROADMAP.md`(+ko)와 `docs/CHAT-REMNANT-REMOVAL-PLAN.md`(+ko)의
  순수 문서 정리.
...
- **`deploy.sh`: 이미지 태그를 더 이상 `values-prod.yaml`에 고정된 값 그대로 믿지 않고
  ...
  시끄럽게 중단한다. 새 의존성 없음(순수 `curl`, 스크립트의 기존 "단순하게" 스타일
  유지)이고 기존 `y`/N 승인 게이트(ADR 0046)도 그대로다 — 스크립트는 지금 뭐가
  존재하는지만 조회할 뿐, 그걸 배포할지는 여전히 사람이 정한다. **같은 날 보완**:
  처음엔 `dev`로 고정돼 있었고 `main`은 `IMAGE_TAG`로만 접근 가능했는데, 개발자의
  실제 의도는 `main`도 같은 배포 기능을 그대로 쓸 수 있어야 한다는 것이었다 —
  `DEPLOY_BRANCH` 환경변수(기본값 `dev`)로 일반화해 두 브랜치가 같은 조회+검증
  로직을 탄다. `IMAGE_TAG`는 이제 두 브랜치의 HEAD가 아닌 것을 쓸 때만의 순수
...
- **`admin/`: 감사 로그가 대상 종류를 `action`에서 역추론하지 않고 서버의 `targetType`을 읽도록
  ...
  일부러 제외한다"고 명시하며 남겨 둔 바로 그 정리다.
  **무엇이 중복이었나.** 두 축이 모두 하루 전인 2026-08-24에 착륙했다. 먼저 콘솔의 표시 결함을
  클라이언트 쪽 `targetLabel(action, targetId)` 매핑으로 고쳤고(아래 항목), 이어 ADR 0045가
  서버 쪽에서 모든 감사 기록에 `targetType` 판별자를 실어 보내기 시작했다. 둘 다 각자 옳았지만,
  합쳐 놓으니 "어떤 action이 어떤 종류를 대상으로 하는가"라는 같은 지식이 두 곳에 놓였다. 오늘의 action 다섯 개에 대해서는
  결과가 틀리지 않는다 — 틀리는 시점은 *나중*이다. 서버에 여섯 번째 action이 추가되면 서버는
  올바르게 표기하고 콘솔은 그 종류를 모르는 상태가 되는데, 그 어긋남을 알려 주는 신호가 어디에도
  없다.
  **무엇을 바꿨나.** `src/lib/audit.ts`의 `AuditLog`에 `targetType: string | null`을 추가하고,
  `action`을 키로 쓰던 `TARGET_NOUN`을 **삭제**한 뒤 서버의 종류를 키로 쓰는 `TARGET_LABEL`로
  대체했다. `targetLabel()`도 action이 아니라 종류를 받는다. ADR 0045 D2가 백엔드에서 세운 원칙,
  즉 "읽기 경로에는 action → 대상 종류 매핑을 두지 않는다"를 클라이언트에도 그대로 적용한 것이다.
  `users-page.tsx`의 "Recent activity" 패널은 행의 대상 종류를 추측할 수밖에 없어 배지와 시각만
  보여 주고 있었는데, 이제 대상을 이름으로 표시한다(`Target: File 169`).
  **두 대안을 제치고 선택했다.** 비교표를 개발자에게 제시해 고른 결과로, 클라이언트 매핑 유지안은
  없애려던 중복 그 자체라서, 서버 우선 + `targetType`이 null일 때 클라이언트 폴백안은 같은 매핑을
  폴백이라는 이름으로 살려 두는 데다 그것이 대비하는 행이 현재 존재할 수 없어서 각각 기각했다.
  선택 전에 확인한 것: `log()`는 `targetType`을 필수 인자로 받으므로 호출부 다섯 곳 중 어느 하나도
  빠뜨릴 수 없고, `targetLabel`의 호출자는 정확히 하나이며, 개발 DB는 **114행 전부 백필, NULL 0건,
  불변식 위반 0건**이다. 감수한 비용은 `targetId`는 있는데 `targetType`이 없는 행이 `File 269`가
  아니라 `#269`로 표시된다는 점인데, 그런 행은 마이그레이션 이후 ADR 0045 이전 코드가 써야만
  생기므로 이 저장소에는 더 이상 존재하지 않는다.
  **사용자에게 보이는 스키마 변경**: CSV 내보내기에 `targetType` 열이 추가됐다
  (`id,createdAt,action,actorId,targetType,targetId,detail`). 내보낸 파일만 봐도 그 id가 무엇의
  id인지 알 수 있다. `e2e/logs.spec.ts`의 헤더 단언을 여기에 맞춰 갱신했고, 이것이 **유일한** e2e
  변경이다 — `User {id}`를 찾는 두 단언은 대상이 `ROLE_CHANGE`(`targetType = 'user'`)라 영향이
  없었다. `admin/README.md`(+ko)도 같은 변경에서 정정했다: `userId`를 "actor 또는 target과 매칭"
  으로 설명하던 네 곳이 좁혀진 의미를 서술하도록 바뀌었고, 2026-08-24 결함 기록에는 그때 열어 둔
  백엔드 질문이 해소됐다는 사실을 남겼다.
  검증: `pnpm lint` 0 errors, `tsc -b`, `pnpm test` 19/19, `pnpm e2e` 11/11. 여기에 실제 브라우저
  확인까지 했다 — `FILE_DELETE` 행이 `File 313`/`File 269`로 표시되고, `/logs?userId=269`는 0건을
  반환하며(ADR 0045 이전에는 무관한 file-269 기록이 나왔다), "View all"도 그대로 동작한다.
- **`frontend/`: 모든 화면이 뷰포트 폭에 맞춰 적응하고, 게시글 보드의 유일한 레이아웃 붕괴를
  수정(2026-08-24, 커밋 `d746257`)** — CSS만 변경했고 컴포넌트·JS·API 변경은 없다.
  **측정이 뒤집은 것**: 이 작업은 "`@media` 블록이 없는 `*.module.css`는 휴대폰에서 깨질
  것"이라는 가정에서 출발했는데, 그 가정이 틀렸다. 손대기 전 390px 뷰포트에서 재보니 **실제로
  넘친 화면은 게시글 보드 하나뿐**이었고(265px 초과), `/files`와 두 상세 페이지, `/login`은
  이미 정상이었다. `#root`가 `max-width: 100%`이고 모든 페이지가 `max-width` 기반이기
  때문이다. 그래서 이 작업은 처음 잡았던 전면 재작성이 아니라, 실제 붕괴 한 곳을 고치고 폭
  구간을 다듬는 일이 됐다.
  붕괴의 원인: `PostBoard.module.css`의 `.creatorButton`이 말줄임 없이 `flex: none`을 달고
  있어, 긴 작성자 이메일이 행의 min-content를 591px로 고정했고 **그 최소폭이 페이지 자체까지
  전파**되어 NavBar·제목·폼을 함께 밀어냈다. 후보 수정안 둘을 브라우저에서 직접 시험했으나
  모두 실패했다 — `width: 100%`도 `min-width: 0`도 `white-space: nowrap` 텍스트의 intrinsic
  폭은 줄이지 못한다 — 그리고 실제로 통한 수정은 제목 링크와 이메일 **둘 다** 줄바꿈시키는
  것이었다. 한쪽만 줄바꿈하면 페이지는 여전히 77px을 넘겼다. 그래서 모바일에서는 말줄임 대신
  줄바꿈(`overflow-wrap: anywhere`)을 쓴다. 이는 단순히 오버플로를 피하려는 선택이 아니라
  가독성 판단이다 — 휴대폰은 세로 공간이 넉넉하므로 제목과 이메일을 잘라 보여주기보다 전문을
  보여주는 편이 낫다. 데스크톱 행은 한 줄 배치와 말줄임을 그대로 유지한다 — 넓은 화면에서는
  잘라 보여주는 쪽이 여전히 맞는 방식이다.
  breakpoint는 세 번째 값을 새로 만들지 않고 **이미 코드베이스에 있는 두 값을 재사용**했다.
  `index.css`가 이미 1024px에서 타입 스케일을 바꾸고, 640px은 프리뷰 그리드가 더 이상 의미를
  갖지 못하는 지점이다. `FileBoard.module.css`의 그리드는 이 두 값을 기준으로 3열 → 2열 →
  1열로 내려가며, 프레임은 모든 단계에서 충분히 크다 — 데스크톱 346×196, 태블릿 구간
  368×208(같은 폭을 두 열이 나누므로 데스크톱보다 크다), 휴대폰 589×332. 다섯 화면 모두 좌우
  여백을 24px → 16px로, 위 여백을 `5vh` → `2vh`로 줄였고(`DashboardPage`/`FileDetailPage`/
  `PostDetailPage`/`PostBoard`/`LoginPage`, 그리고 `UploadForm`의 안쪽 여백), `NavBar.module.css`는
  간격을 좁혔으며, 두 보드 모두 필터 필드를 전체 폭으로 세로 배치하고, 게시글 행은 제목 위·
  작성자 아래로 쌓인다.
  작업 중 새로 만든 결함 2건은 라이브 검증에서 발견해 같은 작업에서 고쳤다: 작성자 ID 입력이
  필터 패널을 9px 넘어간 것(`content-box` 입력에 퍼센트 폭을 주면 padding과 border가 그 위에
  더해지므로 `width: auto`가 맞다), 그리고 줄바꿈된 게시글 제목이 `#root`의
  `text-align: center`를 상속해 왼쪽 정렬된 이메일과 어긋난 것.
  화면 5개 × 폭 5종에서 가로 오버플로 0을 확인했고, 1280px에서 데스크톱 행 배치가 그대로임을
  확인했으며, `pnpm build`/`pnpm lint` 통과와 Playwright e2e 22개 전부 통과를 확인했다. 터치
  타겟 크기는 의도적으로 범위 밖에 뒀다 — [ROADMAP.ko.md](ROADMAP.ko.md) > 7 참고.
- **`frontend/`: 파일 보드를 3열 프리뷰 그리드 + 무한 스크롤로 개편(2026-08-24, 커밋
  ...
  단일 열 폴백은 새 그리드가 휴대폰에서 최소한 쓸 수 있게 하는 선까지만 넣은 것이며,
  이를 대체한 반응형 작업은 2026-08-24에 완료됐다(위 항목).
