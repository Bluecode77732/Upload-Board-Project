...
- ~~앱 크기/용량 최적화~~ — **2026-09-16 재검토: 불필요.** 두 가지를 가정이 아니라 직접
  확인했다 — 업로드 파일 저장 용량, 그리고 앱 자체의 코드/이미지 크기. 저장 용량 쪽은
  [ADR 0049](ADR/0049-performance-capacity-criteria.ko.md) D4에서 이미 절대 디스크 상한 대신
  사용률 기반 모니터링을 택하기로 결정했고, 그 판단을 바꿀 변화는 없다 — 여전히 실배포된
  AWS가 없고 `file/upload`/`file/temp`/dev DB 모두 비어 있어 최적화 대상이 될 실사용
  데이터 자체가 없다. 코드/이미지 크기 쪽: 의존성은 이미 가볍고(`frontend/` production
  패키지 3개, `admin/` 7개, `backend/`에 무거운 라이브러리 없음), 빌드 산출물도
  작으며(`dist` 1.7M, `frontend/dist` 306K, `admin/dist` 342K), `Dockerfile`은 이미
