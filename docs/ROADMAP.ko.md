...
| **Kubernetes** | 오케스트레이션 | 🔶 매니페스트 | `k8s/` 아래 기본 매니페스트(Pod, Deployment, ClusterIP Service, rolling-update) — 2026-08-17 아래 Helm 적응과 함께 `nginx` placeholder 내용을 바로잡아 이제 프로젝트 전용(`app: upload-board-api`, `bluecode1775/sharenpo:latest`, 포트 3000)이다. **실제 클러스터 배포**(AWS)는 여전히 미착수. | 커밋 `48a89f2`, [0041](ADR/0041-helm-chart-project-adaptation.ko.md) |
...
| **Helm** | 릴리스 패키징 | 🔶 프로젝트 적응 완료 | 2026-08-17 프로젝트 전용으로 적응([0041](ADR/0041-helm-chart-project-adaptation.ko.md), [0037](ADR/0037-helm-chart-scaffold.ko.md)의 유예 해제): 실제 이미지/포트, `/health/live`+`/health/ready` probe, non-root `securityContext`, `ConfigMap`, `existingSecret` 전용 `Secret` 소비, `docker-compose.yml`의 `migrate` 서비스를 본뜬 migration `Job`, 기본 비활성 `Ingress`. `replicaCount` 기본값은 1(`STORAGE_DRIVER=local`에서 1보다 크면 replica 간 업로드 파일이 사라짐). `helm lint --strict`/`helm template`으로만 검증 — 남은 일: **살아있는 클러스터**에 대한 `helm install`, 아직 클러스터 자체가 없음. | [0037](ADR/0037-helm-chart-scaffold.ko.md), [0041](ADR/0041-helm-chart-project-adaptation.ko.md) |
