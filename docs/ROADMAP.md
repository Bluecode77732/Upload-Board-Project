...
| **Kubernetes** | Orchestration | 🔶 manifests | Base manifests under `k8s/` (Pod, Deployment, ClusterIP Service, rolling-update) — now project-specific (`app: upload-board-api`, `bluecode1775/sharenpo:latest`, port 3000) after their `nginx` placeholder content was corrected 2026-08-17 alongside the Helm adaptation below. A **live cluster deploy** (on AWS) is still not done. | commit `48a89f2`, [0041](ADR/0041-helm-chart-project-adaptation.md) |
...
| **Helm** | Release packaging | 🔶 project-adapted | Project-adapted 2026-08-17 ([0041](ADR/0041-helm-chart-project-adaptation.md), lifting [0037](ADR/0037-helm-chart-scaffold.md)'s deferral): real image/port, `/health/live`+`/health/ready` probes, non-root `securityContext`, a `ConfigMap`, `existingSecret`-only `Secret` consumption, a migration `Job` mirroring `docker-compose.yml`'s `migrate` service, and a disabled-by-default `Ingress`. `replicaCount` defaults to 1 (`STORAGE_DRIVER=local` loses uploaded files across replicas above 1). `helm install --wait` verified against a throwaway local `kind` cluster (2026-08-17 addendum to [0041](ADR/0041-helm-chart-project-adaptation.md)) — found and fixed 2 real bugs (hook ordering, empty-string env vars). Remaining: install against a **real target cluster (AWS/EKS)**, which does not exist yet. | [0037](ADR/0037-helm-chart-scaffold.md), [0041](ADR/0041-helm-chart-project-adaptation.md) |
