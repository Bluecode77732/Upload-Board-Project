...
| **Kubernetes** | Orchestration | 🆕 not currently deployed | The standalone static manifests formerly under `k8s/pod/`, `k8s/deployment/`, `k8s/cluster/` were deleted 2026-08-17 ([0042](ADR/0042-k8s-helm-directory-consolidation.md)) — they duplicated a strict subset of what the Helm chart below already renders, with no consumer of their own (no CI job, no compose reference). The Kubernetes manifests now exist only as the Helm chart's `templates/` (`k8s/helm/`). **The live cluster deploy (on AWS) landed 2026-08-27** (§9) — `helm upgrade` reached `STATUS: deployed`, migration Job completed, app pod `Running`/ready — **then the underlying cluster was destroyed 2026-08-28** (§9) once that was proven; nothing is running now. `bash k8s/infra/terraform/deploy.sh all` reproduces it. | commit `48a89f2`, [0041](ADR/0041-helm-chart-project-adaptation.md), [0042](ADR/0042-k8s-helm-directory-consolidation.md) |
| **Secrets delivery** | Secrets | 🔶 code ✅ / currently not running | Target decided: native **Kubernetes `Secret`**, synced from **AWS Secrets Manager** by External Secrets Operator via IRSA. The Helm chart implements the consumption side (`existingSecret` reference + `envFrom.secretRef`, 2026-08-17); the Terraform side ([0043](ADR/0043-terraform-project-adaptation.md) D7, 2026-08-18) provisions the Secrets Manager entry and ESO's install + IRSA role (via `eks_blueprints_addons`'s `enable_external_secrets`). **Confirmed working live 2026-08-27**: `secretstore.external-secrets.io/upload-board-project-secretsmanager` was `Valid`/`Ready`, and `externalsecret/upload-board-project-app-secrets` was `SecretSynced`/`True` — the app's running pod authenticated against RDS and signed tokens using exactly this synced Secret. **Destroyed along with the rest of the stack 2026-08-28** (§9) — nothing currently syncs; the mechanism is proven, not standing. | [0033](ADR/0033-secrets-delivery-target.md), [0041](ADR/0041-helm-chart-project-adaptation.md), [0043](ADR/0043-terraform-project-adaptation.md) |
...
| **AWS** | Cloud / deploy target | 🆕 proven, not currently running | The container deploy target the rows above build toward — proven end-to-end 2026-08-25–27: account `074416822640` (`sharenpo-user`, Paid Plan since 2026-08-27), region `ap-northeast-2`, live EKS + RDS + S3 + Route53/ACM, with the app itself deployed and running (§9, 2026-08-27). **Fully torn down 2026-08-28** once proven, to stop the bill (§9) — currently nothing runs on AWS for this project; re-running `deploy.sh all` is the same, already-proven path back. | deployment ADR (planned) |
