...
| **Kubernetes** | Orchestration | 🔶 manifests via Helm | The standalone static manifests formerly under `k8s/pod/`, `k8s/deployment/`, `k8s/cluster/` were deleted 2026-08-17 ([0042](ADR/0042-k8s-helm-directory-consolidation.md)) — they duplicated a strict subset of what the Helm chart below already renders, with no consumer of their own (no CI job, no compose reference). The Kubernetes manifests now exist only as the Helm chart's `templates/` (`k8s/helm/upload-board-project/`). A **live cluster deploy** (on AWS) is still not done. | commit `48a89f2`, [0041](ADR/0041-helm-chart-project-adaptation.md), [0042](ADR/0042-k8s-helm-directory-consolidation.md) |
| **Secrets delivery** | Secrets | 📝 design-only | Target decided: native **Kubernetes `Secret`**; **AWS Secrets Manager** deferred to the Terraform step. The Helm chart now implements the target shape (`existingSecret` reference + `envFrom.secretRef`, 2026-08-17) — the ESO/AWS Secrets Manager layer itself is still not built. | [0033](ADR/0033-secrets-delivery-target.md), [0041](ADR/0041-helm-chart-project-adaptation.md) |
| **HTTPS termination** | TLS | 📝 design-only | Terminate at **ingress / ALB**, never in-process (the `Secure` refresh cookie needs it when `ENV=prod`). The Helm chart's `Ingress` template exists but stays disabled by default — no code path to enable it yet (needs a live cluster, DNS, and a cert mechanism). | [0034](ADR/0034-https-termination-stance.md), [0041](ADR/0041-helm-chart-project-adaptation.md) |
| **Helm** | Release packaging | 🔶 project-adapted | Lives at `k8s/helm/upload-board-project/` (moved from a sibling `helm/` directory 2026-08-17, [0042](ADR/0042-k8s-helm-directory-consolidation.md), so Kubernetes-related content has one top-level home). Project-adapted 2026-08-17 ([0041](ADR/0041-helm-chart-project-adaptation.md), lifting [0037](ADR/0037-helm-chart-scaffold.md)'s deferral): real image/port, `/health/live`+`/health/ready` probes, non-root `securityContext`, a `ConfigMap`, `existingSecret`-only `Secret` consumption, a migration `Job` mirroring `docker-compose.yml`'s `migrate` service, and a disabled-by-default `Ingress`. `replicaCount` defaults to 1 (`STORAGE_DRIVER=local` loses uploaded files across replicas above 1). `helm install --wait` verified against a throwaway local `kind` cluster (2026-08-17 addendum to [0041](ADR/0041-helm-chart-project-adaptation.md)) — found and fixed 2 real bugs (hook ordering, empty-string env vars). Remaining: install against a **real target cluster (AWS/EKS)**, which does not exist yet. | [0037](ADR/0037-helm-chart-scaffold.md), [0041](ADR/0041-helm-chart-project-adaptation.md), [0042](ADR/0042-k8s-helm-directory-consolidation.md) |
