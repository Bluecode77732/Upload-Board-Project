...
### 추가 기록 (2026-09-22) — B안의 비용 주장을 수치로 확인

Alternatives rejected의 B는 서브도메인별 ALB가 "A보다 AWS 쪽 변경이 더 든다"고만 적었다 —
정성적일 뿐 실제 가격은 따진 적이 없었다. 직접 질문받아 AWS 공식 요금 페이지로 확인했다
(기억이 아니라): ALB는 두 가지로 과금된다 — ALB가 떠 있는 시간당 고정 요금("ALB가 가동
중인 시간(또는 그 일부)마다 과금된다", `us-east-1` 기준 시간당 약 US$0.0225), 그리고 네
차원(신규 연결, 활성 연결, 처리 바이트, 규칙 평가) 중 가장 높은 값으로 매기는 LCU
종량 요금 — "가장 사용량이 높은 차원 하나로만 과금한다." 경로 규칙·타깃 그룹·타깃 수 자체는
과금 대상이 아니며, 규칙 평가는 시간당 "처음 처리되는 규칙 10개까지 무료"인데 이 차트의
Ingress는 8개뿐이다.

그래서 D1(같은 ALB) 설계가 B 대비 갖는 구체적이고 고정된 절감은 하나다: **ALB 시간당
요금이 1개분이지, 2개분이 아니라는 것**이다 — B는 서브도메인용 ALB를 하나 더 띄워야 해서
그 고정 비용이 두 배가 된다. 그 외 차원(LCU, 데이터 전송)은 어느 쪽이든 실제 트래픽으로
정해지는 종량제라 거의 같다 — 어느 ALB가 그 트래픽을 처리하느냐로 달라지지 않는다. B의
설계가 실제로 배로 늘리는 건 시간당 고정 요금뿐이다. 이건 "더 든다"던 정성적 서술을
수치로 확인해 준 것일 뿐, 결정의 어떤 부분도 바꾸지 않는다.

### 추가 기록 (2026-09-25) — 후속 작업 7 해소

후속 작업 7(`admin/` 호스팅)은 [ADR 0062](0062-admin-same-alb-subpath-routing.ko.md)로
해소됐다: 같은 메커니즘으로, 같은 Ingress의 `/admin` 규칙이 가리키는 세 번째 values 게이팅
