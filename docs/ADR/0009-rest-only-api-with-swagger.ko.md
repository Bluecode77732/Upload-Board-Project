...
API 표면은 작고(컨트롤러 4개, 라우트 약 13개) 단순한 요청-응답형입니다 — CRUD와
2단계 업로드. GraphQL, WebSocket, gRPC는 각각 스키마 계층, 클라이언트 연동 부담,
운영 복잡성을 더하지만, 현재 기능 중 이를 필요로 하는 것이 없습니다. 또한 API는
프론트엔드 없이 수동 테스트가 가능해야 합니다.
...
- 하나의 프로토콜, 하나의 문서 소스. `/doc`은 수동 테스트 환경을 겸합니다
...
- 모든 엔드포인트 변경에는 데코레이터 의무가 따릅니다 — 리뷰에서 검증합니다
  ...
  기술하는지 검증").
- 실시간·스트리밍 기능은 없습니다. 그 필요가 생긴다면 점진적 확장이 아니라 명시적
  아키텍처 변경으로 다룹니다.
