...
**두 숫자(24시간 vs 1시간)는 애초에 일치할 필요가 없고, 일치해야 한다는 전제로
비교하는 건 서로 다른 종류의 값을 비교하는 것이다.** `TEMP_SWEEP_TTL_HOURS`는
**사람/클라이언트 시간 척도**의 대기다 — 실제 사용자가 2단계 업로드의 후반부를
끝내는 데 얼마나 걸릴 수 있는지(브라우저를 닫거나, 딴짓하거나, 느리게 재시도하거나).
`MIN_AGE_MS`는 **서버 내부 시간 척도**의 레이스 여유다 — 트랜잭션 커밋 하나가
최대 얼마나 걸릴 수 있는지. 후자는 정상 동작 시 밀리초 단위로 끝나므로, 1시간이면
이미 엄청나게 넉넉하다; 이걸 24시간 TTL에 맞춰 늘려도 안전성이 늘지 않는다 — 이미
결정이 끝난 정리(완료된 삭제인데 `unlink()`만 실패한 것)나, 호출자 입장에서 이미
실패한 업로드 시도의 잔해를 회수하는 시점만 괜히 늦출 뿐이다 — 이 비교에서 드러난
세 번째 고아 발생 경로는 아래 결과(Consequences)의 잔여 사항 노트 참고.

그 TTL과 달리 이 기준은 설정값으로 노출하지 **않는다** — 정정 전 초안엔
...
- `GRANTED_SWEEP_CRON` (string, 기본값 `'0 0 * * *'` — 매시간이 아니라 매일 자정: DB 조인은
...
- 세 번째 고아 발생 경로 — 원래 설계 단계가 아니라 D3의 기준을 ADR 0018의 TTL과
  비교하다가 드러났다: `FileService.uploadFile`은 행을 insert하고, `storage.promote()`를
  호출하고, 커밋하는 순서를 전부 `try` 하나 안에서 한다(`file.service.ts`) —
  `promote()`는 성공했는데 그 직후 `commitTransaction()`이 실패하면(DB 연결 끊김,
  데드락) `catch`가 insert를 롤백하지만 물리 이동은 이미 끝나버려서, 영구히 행 없는
  `granted_` 파일이 남는다. 이건 "사용자가 쓰던 파일"이 아니다 — 그 시점에 호출자
  자신의 요청은 이미 500으로 실패했으므로, 그 파일이 `MIN_AGE_MS`를 넘길 즈음엔
  호출자 입장에서 "진행 중인" 게 아무것도 없다. 기존 D3 selector로 코드 변경 없이
  이미 커버된다; 이 ADR의 배경이나 ROADMAP의 원래 두 가지 목록 어디에도 이름 붙여진
  적이 없어서 여기 기록해둔다.
