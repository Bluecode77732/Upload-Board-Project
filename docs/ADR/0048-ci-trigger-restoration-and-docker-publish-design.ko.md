...
### Addendum (2026-08-31) — 다음 날 발견·해결한 설계 갭 5가지

다음 날 재검토에서 이 ADR 자체의 설계 갭 5가지가 드러났다(별도 언급 없는 한
...
- **Docker Hub 태그 무한 증식.** `dev`가 이제 push마다 `:<sha>` 태그를
  ...
  `.github/workflows/docker-tag-cleanup.yml`로 해결: 매일(그리고
  ...
  예약된 cron 실행은 항상 실제로 삭제한다. **`on.schedule`엔 `branches:`
  필터가 없다** — 이건 이 워크플로 설정의 허점이 아니라 GitHub Actions
  플랫폼 자체의 단단한 제약이다: cron 트리거는 브랜치가 뭐라고 적혀 있든
  항상 저장소 기본 브랜치(`main`)에 있는 파일 버전으로만 실행되고,
  `dev`에서는 절대 실행되지 않는다. 훗날 `dev`→`main` 병합으로 이 파일이
  `main`에 실제로 반영되기 전까지는 매일 cron 항목이 죽어 있는 상태이고,
  그전에 더 일찍 돌리는 유일한 방법은 `--ref dev`를 명시한
  `workflow_dispatch`뿐이다. 정리 로직 자체는 애초에 브랜치를 인지하지
  않는다 — Docker Hub 태그 목록을 전역으로 훑으므로, 어느 브랜치의 파일로
  실행하든 어느 브랜치가 만든 sha 태그든 똑같이 정리 대상이다. **아직
  실제로 실행해보진 않았다** — `main`에 도달한 뒤의 첫 예약 실행이나, 그전에
  `dev` 대상으로 돌리는 수동 드라이런이 이 스크립트의 첫 실제 검증이다.
  Docker Hub 삭제 API를 실제로 호출하지 않고는 안전하게 검증할 방법이
  없었다.
...
- **스모크 테스트가 마이그레이션을 한 번도 실행한 적이 없어, 깨진
  마이그레이션을 못 잡았다.** (같은 Addendum에서 위에 추가한)
  `GET /health/ready`는 그냥 `SELECT 1`(`HealthService.checkDatabase`)로
  DB를 핑할 뿐이라 — 어떤 테이블도 쿼리하지 않으므로 완전히 마이그레이션된
  DB든 텅 빈 미마이그레이션 DB든 똑같이 성공한다. 원래 스모크 테스트도,
  위에서 추가한 `/health/ready` 수정도, 스모크 테스트 컨테이너가 연결하는
  바로 그 임시 Postgres에 `pnpm run migration:run`을 실행한 적이 없었다 —
  그래서 마이그레이션 자체가 완전히 실패하는 경우(문법 오류, 또는 베이스라인
  스키마와의 충돌)가 CI에서 전혀 드러나지 않고, 실제 배포 시점에 Helm
  차트의 별도 마이그레이션 `Job`([ADR 0032](0032-migration-as-separate-deploy-step.ko.md))에서만
  터졌을 것이다. `docker-publish`에 `actions/setup-node` +
  `pnpm install --frozen-lockfile` + `pnpm run migration:run` 스텝을
  추가했다(기존 `e2e`/`frontend-e2e`/`admin-e2e` 잡들의 셋업을 그대로
  본뜸) — 이미지를 빌드하기도 전에, 스모크 테스트 컨테이너가 쓰는 것과
