...
| **S3** | Object storage | 🔶 adapter ✅ / redirect design 📝 / cutover 🆕 | The `FileStorage` port + `S3Storage` implementation landed (unit-tested only, never run against a live bucket). The proxy-streaming path was bandwidth-heavy on the app tier, so a presigned-URL redirect for `GET /file/:id/content` (all three visibility tiers, gated by the existing `resolveContentAccess` check) is now designed — no code yet. Remaining: implement the redirect, then the real-bucket **`STORAGE_DRIVER=s3` cutover**. `local` stays the operative default. | [0029](ADR/0029-storage-port-adapter.md), [0036](ADR/0036-s3-presigned-content-redirect.md) |
