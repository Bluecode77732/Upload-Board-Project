...
- **`log-session-title.js`** (`UserPromptSubmit`, 같은 날 추가 — session_id 하나만으로는
  ...
  프롬프트마다 실행되어, 해당 세션의 가장 최근 **빈 제목** 행을 찾아 프롬프트를 약
  80자로 잘라 평탄화한 값으로 채운다. 한 번 채워진 행은 더 이상 빈 제목 정규식에
  매치되지 않으므로 행당 최대 한 번만 쓴다. 원래는 `turn_number === 1`로 게이팅했으나,
  같은 날 실제 로그 데이터에서 **재개된 세션의 행은 영원히 빈 채로 남는 것**을 확인했다 —
  재개는 턴 카운트를 리셋하지 않고 이어가므로 재개 후 첫 프롬프트가 turn 1이 되는 일이
  없기 때문이다. 그래서 게이트를 "빈 제목 행 매치" 자체로 대체했고, 이는 신규 세션의
  진짜 첫 프롬프트와 재개된 세션의 재개 후 첫 프롬프트 양쪽 모두에서 올바르게 채워진다.
