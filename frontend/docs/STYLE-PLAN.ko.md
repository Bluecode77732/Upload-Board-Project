...
4. **모션 전무, 깊이는 한 곳뿐** (2026-08-25 기록, 3번과 함께) —
   **2026-09-07 기준 여전히 열려 있음**: 3번의 타이포그래피 세션은 개발자가
   실제로 요청한 마크 + 헤딩 서체로만 범위를 좁혔고, 이 항목은 의도적으로
   함께 처리하지 않았다. `transition`·`animation`·`@keyframes`가 `src/`의
   모든 스타일시트에 **0회** 등장해 hover·focus가 전부 즉시 점프하고,
   `--shadow`는 정확히 한 곳(`LoginPage.module.css`의 카드)에만 적용돼
   나머지 표면은 모두 평면이다. 둘 다 토큰으로 한 번에 넣을 수 있을 만큼
   저렴하다.
