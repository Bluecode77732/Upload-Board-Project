...
2. 위 "범위 밖이지만 관련된 사항"에 남은 2건을 이후 별도 작업으로 진행할지,
   의도적으로 그대로 둘지 확인.
3. ~~**타이포그래피는 사실 결정된 적이 없다**~~ (2026-08-25 기록) —
   **2026-09-07 해결**. 탐구는 비교표 기반 Q&A(CLAUDE.md의 확인 절차)로
   시작해, 6개 헤딩 서체 후보(시스템 세리프 폴백 + Fraunces·Bricolage
   Grotesque·Instrument Serif·Unbounded·Manrope 5개 웹폰트)를 실제
   `LoginPage` 카드 위에 라이트/다크 두 테마로 렌더링한 아티팩트 프리뷰
   페이지로 이어졌다. **확정: 시스템 세리프 폴백** —
   `--heading: ui-serif, Georgia, 'Times New Roman', serif`(기존에는 `--sans`와
   바이트 단위로 동일했다). 신규 의존성이 0건이라 `frontend/CLAUDE.md`의
   의존성 사전 제안 게이트가 아예 걸리지 않는다 — 웹폰트 후보 5개보다 위험이
   가장 낮은 안이 선택됐다. `--sans`는 그대로 유지.
4. ~~**모션 전무, 깊이는 한 곳뿐**~~ (2026-08-25 기록, 3번과 함께) —
   **2026-09-08 해결**. 3번의 타이포그래피 세션은 마크 + 헤딩 서체로만
   범위를 좁히면서 이 항목을 의도적으로 미뤄뒀는데, 이번 작업에서 별도의
   디자인 결정 없이 코드로 바로 처리했다. `index.css`에 `--transition:
   all 150ms ease;` 토큰 하나를 추가하고, 이미 `:hover`/`:focus-visible`
   상태를 가진 기존 버튼 클래스 전부에 `transition: var(--transition);`을
   적용했다 — `*.module.css` 12개 파일에 걸친 24개 셀렉터로, `:focus-visible`
   키보드 포커스 작업(커밋 `4e6e229`)이 이미 손댔던 버튼 클래스 목록을 거의
   그대로 포함하는 범위다. `--shadow`(기존에는 `LoginPage.module.css`의 `.card`에만
   적용)는 카드형 표면 중 선별한 일부로 확장했다 — 주요 액션 폼 패널인
   `UploadForm`·`PostForm`·`CommentForm`의 `.form`, 그리고
   `PostDetailPage`의 `.editForm`. `.playerWrapper`·`.tile`·`.item`·필터
   바 같은 수동적 표시 표면은 의도적으로 평면으로 남겼다 — `--shadow`를
   전부에 확장하면 선별이 아니라 과함으로 읽히기 때문이다. 육안 확인
   (Playwright, `getComputedStyle`)으로 검증: 샘플 버튼에서
   `transitionProperty: "all"`, `transitionDuration: "0.15s"`; 확장된 샘플
   폼 카드에서 `boxShadow` 적용 확인.
5. ~~**로그인 마크가 스타터 템플릿의 로고다**~~ (2026-08-25 기록, 3번과 묶음)
   — **2026-09-07 해결**. 탐구는 3번과 같은 방식(비교표 → 아티팩트 프리뷰
   페이지)으로 진행됐고, 개발자가 계속 더 많은 후보를 요청하면서 원래의
   "새로 디자인 vs 워드마크만" 이분법을 크게 벗어났다: 총 19개 마크(A–S) —
   무료 MIT/ISC 라이선스 아이콘(Lucide, D–P), 완전 오리지널 손그림 도형
   (B, Q–S), 각지고 "성장하는" 방향(T–V), 기존 `--brand` 퍼플을 실제
   사파이어 커팅과 연결하는 보석 방향(W–Y)까지 포함한다. 배지 처리는 먼저
   결정됐다 — 배지 없음(채우거나 둥글리거나 원형 컨테이너에 담지 않은,
   `--brand` 색 선 아이콘 하나). **확정: 마크 S** — 겹친 원 두 개
   (채우기 없이 `var(--brand)` 선만), 업로더와 열람자 사이의 추상적
   "연결"이자 부드럽게 보면 이 앱 이름의 이니셜로도 읽힌다. `LoginPage.tsx`는
   이제 `<img src="/favicon.svg">` 대신 SVG를 직접 인라인해(라이트/다크
   `--brand` 전환에 선 색이 따라가도록) 렌더링하며, `favicon.svg`는 같은
   겹친 원 형태를 고정 색상으로 담는다(`prefers-color-scheme`를 파비콘에서도
   지원하는 브라우저를 위한 `<style>` 블록 + 그렇지 않은 브라우저를 위한
   평면 폴백) — 브라우저 탭은 페이지 CSS 커스텀 프로퍼티를 볼 수 없기
   때문이다. `icons.svg`는 삭제됐다.
